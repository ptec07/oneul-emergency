...
export default function App() {
  ...
  async function runEmergencySearch(location: SearchLocation) {
    ...
    try {
      const response = await fetchEmergencyRooms({ ...location, radiusM: EMERGENCY_SEARCH_RADIUS_M })
      setResult(response)
    } catch {
  ...
  async function runPharmacySearch(location: SearchLocation) {
    ...
    try {
      const response = await fetchOpenPharmacies({ ...location, radiusM: PHARMACY_SEARCH_RADIUS_M })
      setResult(response)
    } catch {
      setErrorMessage(ERROR_MESSAGE)
    } finally {
